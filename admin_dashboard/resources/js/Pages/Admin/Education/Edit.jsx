...
export default function Edit({ material }) {
  const { data, setData, put, processing, errors } = useForm({
    ...
    category: material.category || 'pengetahuan_isk',
    hbm_component: material.hbm_component || 'perceived_susceptibility',
    content: material.content || '',
    video_url: material.video_url || '',
    thumbnail_url: material.thumbnail_url || '',
    published: material.published ?? 1,
    sort_order: material.sort_order ?? 1,
  });

  const handleSubmit = (e) => {
    e.preventDefault();
    put(`/admin/education/${material.id}`);
  };

  return (
    <AdminLayout title={`Edit Materi - ${material.title}`}>
      ...
      <div className="max-w-3xl mx-auto space-y-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-bold text-slate-800">Edit Materi Edukasi</h2>
            <p className="text-xs text-slate-400">ID Materi: {material.id}</p>
          </div>
        </div>
