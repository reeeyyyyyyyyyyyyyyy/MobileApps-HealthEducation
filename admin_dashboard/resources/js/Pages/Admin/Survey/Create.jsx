...
export default function Create({ defaultType, defaultVariable }) {
  const { data, setData, post, processing, errors } = useForm({
    type: defaultType || 'pretest',
    variable: defaultVariable || 'pengetahuan',
    question_text: '',
    question_order: 1,
    is_active: true,
  });
  ...
  return (
    <AdminLayout title="Tambah Soal Kuesioner">
      <Head title="Tambah Soal Kuesioner - UtiCare" />
      ...
      <div className="max-w-2xl mx-auto space-y-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-bold text-slate-800">Tambah Soal Kuesioner</h2>
            <p className="text-xs text-slate-400">Instrumen penelitian skala Likert 5 poin</p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm space-y-4">
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Contoh: Infeksi saluran kemih lebih mudah terjadi pada wanita karena uretra wanita lebih pendek."
          ...
          <div className="p-3 bg-violet-50 rounded-xl text-xs text-violet-800">
            * Opsi Skala Likert 5 poin (Sangat Setuju [5] s/d Sangat Tidak Setuju [1]) akan di-generate secara otomatis untuk setiap instrumen baru.
          </div>
